    ...
    <div className="bg-black text-white min-h-screen">
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <nav className="mb-5 flex items-center gap-1 text-xs text-zinc-500">
          ...
          <span>/</span>
          <Link href="/consolas" className="hover:underline hover:text-zinc-300">Consolas</Link>
          <span>/</span>
          <span className="font-medium text-zinc-300">{family.title}</span>
        </nav>

        {/* Hero con imagen de portada */}
        <section className="relative overflow-hidden rounded-xl border border-zinc-800 bg-[#0d0d0d] text-white">
          {cover && (
            <Image
              src={cover.src}
              alt={cover.alt}
          ...
          <div className="relative px-6 py-8 sm:px-10 z-10">
            <h1 className="text-2xl font-brand font-black uppercase tracking-wider text-white sm:text-3xl">{family.title}</h1>
            <p className="mt-2 max-w-xl text-sm leading-relaxed text-zinc-400">
              {family.description}
            </p>
            <p className="mt-3 text-xs text-zinc-500">
              Generaciones: <span className="text-primary font-semibold">{family.generations.join(" · ")}</span>
            </p>
        ...
        {visibleSections.includes("consolas") && (
          <section className="mt-8">
            <h2 className="text-lg font-brand font-bold uppercase tracking-wider text-white">Consolas {family.title}</h2>
            <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {consoles.length ? (
                consoles.map((p) => <ProductCard key={p.id} product={p} />)
              ) : (
                <p className="rounded-xl border border-zinc-800 bg-[#0d0d0d] p-6 text-sm text-zinc-400 w-full sm:col-span-2 lg:col-span-3">
        ...
        {visibleSections.includes("videojuegos") && (
          <section className="mt-8">
            <h2 className="text-lg font-brand font-bold uppercase tracking-wider text-white">Videojuegos {family.title}</h2>
            <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {games.length ? (
        ...
        {visibleSections.includes("accesorios") && (
          <section className="mt-8">
            <h2 className="text-lg font-brand font-bold uppercase tracking-wider text-white">Accesorios {family.title}</h2>
            <div className="mt-4 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {accessories.length ? (
